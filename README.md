...
Both sender and receiver set taskbar/window icon at runtime and include `logo.ico` in build outputs.
...
## CI Build Scope (Current)

GitHub Actions currently builds **Windows only**. macOS build/release is temporarily paused.
...
Current release pipeline uses independent Windows jobs:

- `windows-sender`
- `windows-receiver`

Then it creates a GitHub Release and uploads zipped Windows artifacts automatically.
